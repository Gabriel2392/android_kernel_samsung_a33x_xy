			     ...
			     struct z_erofs_zstd_cfgs *zstd, int size)
{
	...
	wkspsz = ZSTD_DStreamWorkspaceBound(dict_size);
			    ...
			    struct page **pgpl)
{
	...
	insz = rq->inputsize;
	outsz = rq->outputsize;
	stream = ZSTD_initDStream(z_erofs_zstd_max_dictsize, strm->wksp, strm->wkspsz);
	if (!stream) {
		err = -EIO;
		goto failed_zinit;
	}
	...
	do {
		...
		zerr = ZSTD_decompressStream(stream, &out_buf, &in_buf);
		if (ZSTD_isError(zerr) || (!zerr && outsz)) {
			erofs_err(sb, "failed to decompress in[%u] out[%u]: %d",
				  rq->inputsize, rq->outputsize,
				  zerr ? ZSTD_getErrorCode(zerr) : 1);
			err = -EFSCORRUPTED;
			break;
		}
